...
class _ProductQuantityModalState extends State<ProductQuantityModal>
  ...
  List<Widget> buildFormFields() {
    return [
      ...
      // TextFormField(
      //   key: const Key('product_quantity.ice'),
  ...
  void initState() {
    ...
    // _iceController =
    //     TextEditingController(text: q?.additionalIce ?? '0');
    _amountFocusNode = FocusNode();
    _priceFocusNode = FocusNode();
    _costFocusNode = FocusNode();
    //_iceFocusNode = FocusNode();
  ...
  Future<void> updateItem() async {
    ...
    if (widget.isNew) {
      await widget.ingredient.addItem(ProductQuantity(
        ...
        //additionalIce: object.additionalIce!,
      ));
    } else {
  ...
  ProductQuantityObject _parseObject() {
    return ProductQuantityObject(
      ...
      //additionalIce: num.parse(_iceController.text).toString(),
    );
  }
